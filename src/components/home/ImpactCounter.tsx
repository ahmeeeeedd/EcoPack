...
const ImpactCounter: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const incrementCounter = (step: number) => {
      setCounts({
        bottlesSaved: Math.ceil(
          (currentUser.impact.bottlesSaved * step) / steps
        ),
        treesPreserved: Math.ceil(
          (currentUser.impact.treesPreserved * step) / steps
        ),
        co2Reduced: Math.ceil((currentUser.impact.co2Reduced * step) / steps),
        plasticWaste: Math.ceil(
          (currentUser.impact.plasticWaste * step) / steps
        ),
      });
    };
